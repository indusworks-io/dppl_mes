<template>
  <div class="downtime-reason-page p-6">
    <!-- Loading State -->
        ...
        <div class="flex items-center justify-between flex-wrap gap-4">
          <div class="flex items-center gap-3">
            <button
              ...
              title="Back to Downtime Reasons"
            >
            ...
            <h1 class="text-2xl font-bold text-gray-900">
              {{ docname ? doc.doc.downtime_reason || doc.doc.name : 'New Downtime Reason' }}
            </h1>
          </div>
        ...
        <div class="space-y-6 max-w-2xl">
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">
              Downtime Reason <span class="text-red-500">*</span>
            </label>
            <Input
              v-model="doc.doc.downtime_reason"
              type="text"
              placeholder="Enter downtime reason"
              :disabled="!!docname"
            />
            <p v-if="docname" class="mt-1 text-xs text-gray-500">Downtime reason cannot be changed after creation</p>
          </div>
          ...
          <div>
            ...
            <Select
              v-model="doc.doc.category"
              :options="['Unplanned Downtime', 'Planned Downtime', 'Schedule Loss']"
...
<script setup>
import { createDocumentResource, createResource, Input, Select, Checkbox, toast } from 'frappe-ui'
...
// Validation
const validateForm = () => {
  if (!doc.value.doc.downtime_reason || doc.value.doc.downtime_reason.trim() === '') {
    toast.error('Downtime Reason is required')
    return false
  }
  if (!doc.value.doc.category) {
...
const handleSave = async () => {
  ...
  try {
    if (docname.value) {
      ...
      const dataToSave = {
        category: doc.value.doc.category,
      ...
      const dataToSave = {
        doctype: 'Downtime Reason',
        downtime_reason: doc.value.doc.downtime_reason.trim(),
        category: doc.value.doc.category,
        is_active: doc.value.doc.is_active ? 1 : 0
      }
      ...
      if (result) {
        ...
        router.push(`/production/downtime-reasons/${result.name}`)
      }
    }
  } catch (err) {
...
// Navigation
const goBack = () => {
  router.push('/production/downtime-reasons')
}
</script>

<style scoped>
.downtime-reason-page {
  min-height: 100vh;
